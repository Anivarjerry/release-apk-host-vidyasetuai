...
pcm_questions_hi = {
    ...
    "Mathematics Class 11": [
        ...
        ("बिंदुओं (x1, y1) and (x2, y2) के बीच की दूरी का सूत्र है:", "sqrt((x2-x1)^2 + (y2-y1)^2)", "(x2-x1)^2 + (y2-y1)^2", "x2-x1 + y2-y1", "sqrt(x1^2 + y1^2)", "A", "यह मानक यूक्लिडीय दूरी सूत्र है।"),
...
# Write templates SQL
sql_lines.append("-- 1. INSERT INTO global_journey_templates")
for t in templates_en:
    line = f"INSERT INTO public.global_journey_templates (id, title, description, duration_days, category, version, is_active, is_deleted, created_at, updated_at) VALUES ('{t['id']}', '{t['title']}', '{t['description']}', {t['days']}, '{t['category']}', 1, true, false, '{now_str}', '{now_str}') ON CONFLICT (id) DO NOTHING;"
    sql_lines.append(line)
for t in templates_hi:
    line = f"INSERT INTO public.global_journey_templates (id, title, description, duration_days, category, version, is_active, is_deleted, created_at, updated_at) VALUES ('{t['id']}', '{t['title']}', '{t['description']}', {t['days']}, '{t['category']}', 1, true, false, '{now_str}', '{now_str}') ON CONFLICT (id) DO NOTHING;"
    sql_lines.append(line)

sql_lines.append("\n-- 2. INSERT INTO global_journey_tasks")
# Tasks for English Templates
for t in templates_en:
    cat = t["category"]
    ...
    for day in range(1, t["days"] + 1):
        ...
        line = f"INSERT INTO public.global_journey_tasks (id, template_id, day_number, task_title, task_description, task_type, sort_order, is_active, is_deleted, created_at, updated_at) VALUES ('{task_id}', '{t['id']}', {day}, '{title}', '{description}', 'read', 1, true, false, '{now_str}', '{now_str}') ON CONFLICT (id) DO NOTHING;"
        sql_lines.append(line)

# Tasks for Hindi Templates
for t in templates_hi:
    cat = t["category"]
...
sql_lines.append("\n-- 3. INSERT INTO global_journey_mcqs")
# MCQs for English Templates
for t in templates_en:
    cat = t["category"]
    ...
    for day in range(1, t["days"] + 1):
        for q_index in range(1, 21):
            ...
            line = f"INSERT INTO public.global_journey_mcqs (id, template_id, day_number, question_text, option_a, option_b, option_c, option_d, correct_option, explanation, difficulty_level, points, is_active, is_deleted, created_at, updated_at) VALUES ('{mcq_id}', '{t['id']}', {day}, '{q_text}', '{sample_q[1]}', '{sample_q[2]}', '{sample_q[3]}', '{sample_q[4]}', '{sample_q[5]}', '{sample_q[6]}', 'medium', 1, true, false, '{now_str}', '{now_str}') ON CONFLICT (id) DO NOTHING;"
            sql_lines.append(line)

# MCQs for Hindi Templates
for t in templates_hi:
    cat = t["category"]
...
# Write to file
output_path = "d:/VidyaSetu AI/vidyastu_mboile_app/insert_test_data.sql"
with open(output_path, "w", encoding="utf-8") as f:
    f.write("\n".join(sql_lines))

print(f"Generated SQL written to: {output_path}")
